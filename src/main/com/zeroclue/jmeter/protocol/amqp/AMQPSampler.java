...
public abstract class AMQPSampler extends AbstractSampler implements ThreadListener {
	...
	public static final boolean DEFAULT_EXCHANGE_INTERNAL = false;
	...
	protected static final String EXCHANGE_PARAMETERS = "AMQPSampler.ExchangeParameters";
	private static final String EXCHANGE_AUTO_DELETE = "AMQPSampler.ExchangeAutoDelete";
	private static final String EXCHANGE_INTERNAL = "AMQPSampler.ExchangeInternal";
	...
	protected static final String QUEUE_PARAMETERS = "AMQPSampler.QueueParameters";
	...
	protected boolean initChannel() throws IOException, NoSuchAlgorithmException, KeyManagementException {
		...
		if (channel == null) {
			...
			if (queueConfigured) {
				...
				AMQP.Queue.DeclareOk declareQueueResp = channel.queueDeclare(getQueue(), queueDurable(), queueExclusive(),
						queueAutoDelete(), getQueueArguments());
			}
			...
			if (!StringUtils.isBlank(getExchange())) { // Use a named exchange
				...
				AMQP.Exchange.DeclareOk declareExchangeResp = channel.exchangeDeclare(getExchange(), getExchangeType(),
						getExchangeDurable(), getAutoDelete(), getInternal(), getExchangeArguments());
				if (queueConfigured) {
	...
	private Map<String, Object> getExchangeArguments() {
		Map<String, Object> arguments = new HashMap<String, Object>();
		arguments.putAll(getExchangeParameters().getArgumentsAsMap());
		return arguments;
	}
	...
	public Arguments getExchangeParameters() {
		return (Arguments) getProperty(EXCHANGE_PARAMETERS).getObjectValue();
	}

	public void setExchangeParameters(Arguments parameters) {
		setProperty(new TestElementProperty(EXCHANGE_PARAMETERS, parameters));
	}

	public Arguments getQueueParameters() {
		return (Arguments) getProperty(QUEUE_PARAMETERS).getObjectValue();
	}

	public void setQueueParameters(Arguments parameters) {
		setProperty(new TestElementProperty(QUEUE_PARAMETERS, parameters));
	}
